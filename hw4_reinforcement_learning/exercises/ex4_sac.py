...
class SACAgent:
    ...
    def compute_critic_loss(self, obs, act, rew, next_obs, done) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            q_next = torch.min(torch.stack([q1_next, q2_next], dim=1), dim=1)[0]
            q_next -= self.alpha * next_logp
            target_q = rew + self.gamma * (1-done) * q_next

        q1_pred, q2_pred = self.critic(obs, act)
        critic_loss = 0.5 * torch.square(q1_pred - target_q) + \
            0.5 * torch.square(q2_pred - target_q)
    ...
    def compute_actor_loss(self, obs, act_new, logp_new) -> torch.Tensor:
        ...
        q_new = torch.min(torch.stack([q1_new, q2_new], dim=1), dim=1)[0]
    ...
    def compute_alpha_loss(self, logp_new) -> torch.Tensor:
        ...
        entropy_diff = (logp_new + self.target_entropy).detach()
